...
func (r *Repository) SaveManifest(batchID string, manifest domain.Manifest) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if err := atomicJSON(filepath.Join(r.manifestDir, safeName(batchID)+".json"), manifest); err != nil {
		return err
	}
	r.manifestCache[batchID] = &manifest
	return nil
}
...
func (r *Repository) LoadManifest(batchID string) (*domain.Manifest, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if cached, ok := r.manifestCache[batchID]; ok {
		return cached, nil
	}
