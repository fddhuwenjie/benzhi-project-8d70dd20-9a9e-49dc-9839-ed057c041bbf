...
type Repository struct {
	...
	manifestDir string
	quarantine  string
	batchCache  map[string]*domain.RecordingBatch
	mu          sync.RWMutex
}
...
func Open(root string) (*Repository, error) {
	...
	r := &Repository{root: root, batchDir: filepath.Join(root, "batches"), requestDir: filepath.Join(root, "requests"), manifestDir: filepath.Join(root, "manifests"), quarantine: filepath.Join(root, "quarantine"), batchCache: map[string]*domain.RecordingBatch{}}
...
func (r *Repository) SaveBatch(batch *domain.RecordingBatch) error {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	if err := atomicJSON(filepath.Join(r.batchDir, safeName(batch.BatchID)+".json"), batch); err != nil {
		return err
	}
	r.batchCache[batch.BatchID] = batch
	return nil
}
...
func (r *Repository) LoadBatch(id string) (*domain.RecordingBatch, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if cached := r.batchCache[id]; cached != nil {
		return cached, cached.ValidateSnapshot()
	}
	...
	if err := b.ValidateSnapshot(); err != nil {
		return nil, err
	}
	r.batchCache[id] = &b
	return &b, nil
