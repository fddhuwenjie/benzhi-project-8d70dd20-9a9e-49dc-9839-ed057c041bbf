...
func (s *Service) Release(batchID string, cmd ReleaseCommand) (Result, error) {
	...
	return s.execute(batchID, "release", cmd.Meta, cmd, func() (*domain.RecordingBatch, *domain.Manifest, error) {
		...
		if cmd.LockToken != "" {
			s.preflightMu.Lock()
			lk, ok := s.preflights[cmd.LockToken]
			s.preflightMu.Unlock()
			if !ok || lk.Consumed || lk.BatchID != batchID || lk.Revision != b.Revision || lk.AuditHead != auditHead || lk.Actor != cmd.Actor || !lk.ExpiresAt.After(s.now().UTC()) {
				return nil, nil, errors.New("预检锁已失效")
			}
			if cmd.ExpectedManifestDigest == "" {
				return nil, nil, errors.New("必须提供 expected_manifest_digest")
			}
			if cmd.ExpectedManifestDigest != lk.Digest {
		...
		if cmd.LockToken != "" {
			s.preflightMu.Lock()
			lk := s.preflights[cmd.LockToken]
			lk.Consumed = true
			s.preflights[cmd.LockToken] = lk
			s.preflightMu.Unlock()
		}
		return b, &m, nil
	})
