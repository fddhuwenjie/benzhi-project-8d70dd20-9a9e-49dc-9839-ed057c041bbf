...
func (s *Service) PreviewDetailed(batchID string, marks map[string][]domain.RedactionMark) (map[string]any, error) {
	...
	if value, ok := s.previewCache.Load(batchID); ok {
		if cached, valid := value.(map[string]any); valid {
			return cached, nil
		}
	}
	...
	result := map[string]any{"preview": preview, "alerts": alerts}
	s.previewCache.Store(batchID, result)
	return result, nil
}
